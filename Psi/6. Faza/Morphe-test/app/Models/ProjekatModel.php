...
class ProjekatModel extends Model
{
    ...
    public function dohvatiAktivanProjekatZaMenadzera($idMen){
           $db = \Config\Database::connect();
            return $db->table("menadzer_vodi_projekat")->join("projekat", "menadzer_vodi_projekat.idProjekta=projekat.idPro")->where("menadzer_vodi_projekat.idMen",$idMen)->where("menadzer_vodi_projekat.status","Aktivan")->get()->getResult();
    ...
    public function getProgramersForProject($idProjekta){
        ...
        $db = \Config\Database::connect();
        return $db->table('programer_radi_na')->join('programer','programer.idKor=programer_radi_na.idProgramera')->where("idProjekta",$idProjekta)->get()->getResult();
